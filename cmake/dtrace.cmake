#
# Copied from https://github.com/twitter/mysql/blob/master/cmake/dtrace.cmake
#
...
MACRO (DTRACE_HEADER provider header header_no_dtrace)
 ...
 ADD_CUSTOM_COMMAND(
   ...
   COMMAND perl ${CMAKE_CURRENT_SOURCE_DIR}/misc/tracing/dtrace/dheadgen.pl -f ${provider} > ${header_no_dtrace}
   DEPENDS ${provider}
 )
 ENDIF()
ENDMACRO()
...
IF(ENABLE_DTRACE)
  CONFIGURE_FILE(${CMAKE_CURRENT_SOURCE_DIR}/misc/tracing/dtrace/probes_ubitrack_dtrace.d.base
    ${CMAKE_BINARY_DIR}/utUtil/probes_ubitrack.d COPYONLY)
  DTRACE_HEADER(
   ${CMAKE_BINARY_DIR}/utUtil/probes_ubitrack.d
   ${CMAKE_BINARY_DIR}/utUtil/probes_ubitrack_dtrace.h
   ${CMAKE_BINARY_DIR}/utUtil/probes_ubitrack_nodtrace.h
  )
...
FUNCTION(DTRACE_INSTRUMENT target)
  ...
  IF(ENABLE_DTRACE)
    ...
    IF(CMAKE_SYSTEM_NAME MATCHES "SunOS")
      ...
      ADD_CUSTOM_COMMAND(
        ...
        COMMAND ${CMAKE_COMMAND}
          -DDTRACE=${DTRACE}	  
          -DOUTFILE=${outfile} 
          -DDFILE=${CMAKE_BINARY_DIR}/utUtil/probes_ubitrack.d
    ...
    ELSEIF(CMAKE_SYSTEM_NAME MATCHES "Linux")
      ...
      SET(outfile "${CMAKE_BINARY_DIR}/probes_ubitrack.o")
      # Systemtap object
      EXECUTE_PROCESS(
        COMMAND ${DTRACE} -G -s ${CMAKE_BINARY_DIR}/utUtil/probes_ubitrack.d
        -o ${outfile}
        )
    ...
    install(FILES ${CMAKE_BINARY_DIR}/utUtil/probes_ubitrack_dtrace.h DESTINATION "${UBITRACK_INCLUDE_INSTALL_PATH}/utUtil" COMPONENT main)
